...
void va_msg(int verbosity, const char *str, va_list ap)
{
...
void msg(int verbosity, const char *str, ...)
{
  va_list ap;
  va_start(ap, str);
...
unsigned short *make_port_block(int lo, int hi)
{
...
int bind_socket(char *local_addr, char *local_port, int proto)
{
  ...
  {
    rc = setsockopt(fd, SOL_SOCKET, SO_BROADCAST, &o_broadcast,
                    sizeof(o_broadcast));
    if (rc == -1)
...
void get_sock_name(struct sockaddr *sa, int salen, char *host, char *serv,
                   char *name, char *default_name, char *default_serv)
{
  ...
  {
    gai_errno =
        getnameinfo(sa, salen, name, NI_MAXHOST, NULL, 0, flags | NI_NAMEREQD);
    if (gai_errno)
...
/* connect_socket :
   ...
   an unconnected TCP or UDP socket to listen on.
   Examines various global o_blah flags to figure out what-all to do. */
int connect_socket(char *remote_addr, char *remote_port, char *local_addr,
                   char *local_port, int proto)
{
  char host[NI_MAXHOST + 1], serv[NI_MAXSERV + 1],
      remote_host_name[NI_MAXHOST + 1];
  ...
  if ((gai_errno = getaddrinfo(remote_addr, remote_port, &gai_hints, &whereto)))
  ...
  get_sock_name(&sai_remote, x, host, serv, remote_host_name, remote_addr,
                remote_port);
...
int connect_server_socket(char *remote_addr, char *remote_port,
                          char *local_addr, char *local_port, int proto)
{
  struct addrinfo *whereto;
  ...
  if (o_verbose || !local_port)
  {
    ...
    {
      get_sock_name(&sai_local, x, host, serv, NULL, ADDR_STRING(local_addr),
                    ADDR_STRING(local_port));
      errno = 0;
  ...
  get_sock_name(&sai_local, x, host, serv, local_host_name, local_addr,
                local_port);

  if (o_numeric)
    verbose_msg("connect to %s:%s from %s:%s", remote_host, remote_serv, host,
                serv);
  else
    verbose_msg("connect to %s:%s (%s) from %s:%s (%s)", remote_host,
                remote_serv, remote_host_name, host, serv, local_host_name);
...
int test_udp_port(int fd, char *host)
{
  int rc;
  char c = 0;
...
void hex_dump(char dir, int obc, char *buf, int bc)
{
  ...
  while (bc > 0)
  {
    ...
    /* write data */
    for (x = 16, op = &stage[11], a = &stage[61]; bc && x;
         bc--, x--, buf++, obc++)
    {
...
int answer_telnet_negotiation(char *buf, int size)
{
...
int socket_loop()
{
  ...
  { /* i.e. till the *net* closes! */
   ...
   read anymore after being done with the first port!  */
    ...
    {
      debug_msg("reading from stdin up to %d bytes",
                buf_stdin + BUF_SIZE - out_p);
   ...
   datagrams are kinda fun and exercise the reassembler. */
    ...
    {
      debug_msg("reading from net up to %d bytes",
                buf_socket + BUF_SIZE - in_p);
...
// http://web.mit.edu/freebsd/head/contrib/netcat/netcat.c
/* main :
...
int main(int argc, char **argv)
{
  ...
  // simply ignores the signal SIGPIPE. Passing SIG_IGN as handler ignores a
  // given signal ignores it (except the signals SIGKILL and SIGSTOP which
  // can't caught or ignored).
  ...
  for (; argv[optind]; optind++)
  {
    ...
    while (port_count--)
    {
      ...
      if (port_list)
        sprintf(remote_port = remote_port_buf, "%d", port_list[port_count]);
      else
        remote_port = argv[optind];
      ...
      debug_msg("opening socket from port %s to port %s", sock_fd, local_port,
                remote_port);
      sock_fd = connect_socket(remote_addr, remote_port, local_addr, local_port,
                               o_proto);
